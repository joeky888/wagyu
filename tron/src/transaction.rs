...
pub struct TronTransactionParameters<N: TronNetwork> {
    ...
    pub gas_price: TronAmount,
    /// The nonce of the Tron account
    pub nonce: U256,
    /// The transaction data
    pub data: Vec<u8>,
}

/// Represents an Tron transaction signature
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
struct TronTransactionSignature {
...
/// Represents an Tron transaction id
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
/// Represents an Tron transaction
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
impl<N: TronNetwork> Transaction for TronTransaction<N> {
    ...
    fn from_transaction_bytes(transaction: &Vec<u8>) -> Result<Self, TransactionError> {
        ...
        let parameters = TronTransactionParameters {
            receiver: TronAddress::from_str(&hex::encode(&list[3]))?,
            amount: match list[4].is_empty() {
                true => TronAmount::from_u256(U256::zero()),
                false => TronAmount::from_u256(U256::from(list[4].as_slice())),
            },
            ...
            gas_price: match list[1].is_empty() {
                true => TronAmount::from_u256(U256::zero()),
                false => TronAmount::from_u256(U256::from(list[1].as_slice())),
            },
        ...
        match list[7].is_empty() && list[8].is_empty() {
            ...
            false => {
                ...
                Ok(Self {
                    ...
                    parameters,
                    signature: Some(TronTransactionSignature {
                        v: list[6].clone(),
                        r: list[7].clone(),
                        s: list[8].clone(),
                    }),
...
impl<N: TronNetwork> FromStr for TronTransaction<N> {
...
impl<N: TronNetwork> fmt::Display for TronTransaction<N> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "0x{}",
...
mod tests {
    ...
    fn test_new<N: TronNetwork>(transaction: &TransactionTestCase) {
        ...
        let private_key = TronPrivateKey::from_str(transaction.private_key).unwrap();
        let parameters = TronTransactionParameters {
            receiver: TronAddress::from_str(transaction.to).unwrap(),
            amount: TronAmount::from_wei(transaction.value).unwrap(),
            gas: U256::from_dec_str(transaction.gas).unwrap(),
            gas_price: TronAmount::from_wei(transaction.gas_price).unwrap(),
            nonce: U256::from_dec_str(transaction.nonce).unwrap(),
            data: transaction.data.as_bytes().to_vec(),
        };

        let transaction = TronTransaction::<N>::new(&parameters).unwrap();
    ...
    fn test_sign<N: TronNetwork>(transaction: &TransactionTestCase) {
        let expected_signed_transaction = transaction.signed_transaction;
        let private_key = TronPrivateKey::from_str(transaction.private_key).unwrap();
        let parameters = TronTransactionParameters {
            receiver: TronAddress::from_str(transaction.to).unwrap(),
        ...
        assert_eq!(
            private_key.to_address(&TronFormat::Standard).unwrap(),
    ...
    fn test_from_transaction_bytes<N: TronNetwork>(transaction: &TransactionTestCase) {
        let private_key = TronPrivateKey::from_str(transaction.private_key).unwrap();
        let expected_sender = Some(private_key.to_address(&TronFormat::Standard).unwrap());
        let expected_parameters = TronTransactionParameters {
            receiver: TronAddress::from_str(transaction.to).unwrap(),
            amount: TronAmount::from_wei(transaction.value).unwrap(),
            gas: U256::from_dec_str(transaction.gas).unwrap(),
            gas_price: TronAmount::from_wei(transaction.gas_price).unwrap(),
        ...
        let transaction = TronTransaction::<N>::from_transaction_bytes(&signed_transaction_bytes).unwrap();
    ...
    fn test_to_transaction_bytes<N: TronNetwork>(transaction: &TransactionTestCase) {
    ...
    fn test_to_transaction_id<N: TronNetwork>(transaction: &TransactionTestCase) {
        let expected_signed_transaction_hash = transaction.signed_transaction_hash;
        let private_key = TronPrivateKey::from_str(transaction.private_key).unwrap();
        let parameters = TronTransactionParameters {
            receiver: TronAddress::from_str(transaction.to).unwrap(),
    ...
    fn test_to_string<N: TronNetwork>(transaction: &TransactionTestCase) {
        let expected_signed_transaction = transaction.signed_transaction;
        let private_key = TronPrivateKey::from_str(transaction.private_key).unwrap();
        let parameters = TronTransactionParameters {
            receiver: TronAddress::from_str(transaction.to).unwrap(),
